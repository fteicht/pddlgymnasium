...
from pddlgymnasium.rendering import sar_render, slow_sar_render, posar_render, myopic_posar_render
from pddlgymnasium.structs import Type, Predicate, Not, State, LiteralConjunction
...
class PDDLSearchAndRescueEnv(PDDLEnv):
    ...
    def __init__(self, level=1, test=False, render_version="fast"):
        dir_path = os.path.join(os.path.dirname(os.path.realpath(pddlgymnasium.__file__)), "pddl")
...
if __name__ == "__main__":
    ...
    for env_name in ["PDDLSearchAndRescueLevel7"]: #, "MyopicPOSAR"]:
        imgs = []
        env = pddlgymnasium.make(f"{env_name}-v0")
